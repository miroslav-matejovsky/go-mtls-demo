...
// CreateServer builds an mTLS server that presents the chain bundle and requires
// clients to present a certificate signed by the root CA.
	...
	srv := &http.Server{
		...
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			fmt.Printf("[SERVER] Received request over mTLS — version: %s, cipher suite: %s\n",
				certs.TLSVersionName(tlsState.Version), tls.CipherSuiteName(tlsState.CipherSuite))
