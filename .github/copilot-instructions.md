...
**CA signing is done via CSRs.** The canonical issuance flow is: generate a CSR with `ca.CreateServerCSR` / `ca.CreateClientCSR` / `ca.CreateClientCSRForSigner`, then call `authority.SignServerCSR` / `authority.SignClientCSR`. For the TPM case where only a public key is available, `authority.SignClientCertForKey(pub, cn)` signs directly. There are no exported closure types (`SignerFunc`, `ProfiledSignerFunc`) or exported CA factory functions (`CreateCA`, `CreateRootCA`) — the `ca` package's sole CA creation API is `NewSimple` and `NewEnterprise`.
